...
const logInUser = asyncHandler(async(req,res)=>{
    ...
    //find the user
    // verify the password 
    //access and refresh token generate karna hai
    //send cookies
...
// if the user is logged in then we can fetch the user via auth.middleware.js
const getCurrentUser = asyncHandler(async(req, res)=>{
    const currentUser = await req.user /// since in  auth.middlewarre.js  the user is attached to req

    if(!currentUser){
        throw new ApiError(400 , "unable to fetch user")
    }

    res.status(200).json(200,currentUser,"User fetched Successfully!!")

})

//OR WE CAND IMPLEMENT THE BELOW METHODS BOTH ARE ACCURATE
